...
class Beam:
    ...
    def step(self, log_probs, prev_beam,done_predicate):
        #print(log_probs,prev_beam,done_predicate)
        prev_score = np.array(prev_beam.scores, log_probs.dtype)
        #prev_score = log_probs.new_tensor(prev_beam.scores)
        ...
        for score, bid,wid in zip(nbest_score,beam_indices,word_indices):
            ...
            candidates = prev_candidates[int(bid)] + [wid]
